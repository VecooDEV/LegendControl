...
import com.vecoo.legendcontrol_defender.service.PlayerStorage;
...
public class LegendControlService {
    ...
    public static Map<UUID, PlayerStorage> getStorage() {
        return LegendControlDefender.getInstance().getPlayerService().getStorage();
    }

    @NotNull
    public static Set<UUID> getPlayersTrust(@NotNull UUID playerUUID) {
        return LegendControlDefender.getInstance().getPlayerService().getStorage(playerUUID).getPlayersTrust();
    }

    public static boolean hasPlayerTrust(@NotNull UUID playerUUID, @NotNull UUID targetUUID) {
        return LegendControlDefender.getInstance().getPlayerService().getStorage(playerUUID).getPlayersTrust().contains(targetUUID);
    }
