...
public abstract class LegendarySpawnerMixin extends TickingSpawner {
    ...
    public void forcefullySpawn(@Nullable ServerPlayerEntity onlyFocus) {
        ...
        ArrayList<ServerPlayerEntity> players = new ArrayList<>(ExtrasAPI.getInstance().getServer().getPlayerList().getPlayers());
        if (onlyFocus == null) {
            while (!players.isEmpty()) {
                ArrayList<ServerPlayerEntity> cluster = new ArrayList<>();
                ServerPlayerEntity focus = players.remove(0);
                if (!PixelmonConfigProxy.getSpawning().isSpawningDisabledDimension(focus.getLevel()) && !ServerFactory.getPlayersBlacklist().contains(focus.getUUID())) {
